...
* Minor Update (2025). Is there a better C?
    ...
    Nim is fine in my book. It can even be better C/Pascal than Odin. I fear the latter forces everyone into allocators prematurely, like Zig. Also, Odin/Zig's errors as values are good on paper, but we have seen what they do to Go and Rust, with paranoid error checking and manual propagation in every function, which does not guarantee anything, and you still have to know what panics, deal with different error subtypes across libraries, unknown errors, unreachable errors somewhere in the middleware. 
    
    Personally, it is not fun to read or write Pascal, Ada, Go, Zig, Rust, Odin. The languages feel like tools to "get shit done", you know what they are, you can sense their robustness. Nim is different. It can feel like scripting, it invites you to study/polish things, which can also be good or bad.
